...
export default function CustomSwitch({
  ...
  console.log(`render - startOnLeft: ${startOnLeft}`);

  const [toggleRight, setToggleRight] = useState(
    startOnLeft === true ? true : false,
  );

  const colorAnim = useRef(new Animated.Value(0)).current;

  const userefx = useRef(
  ...
  const colorAnimInterpolation = onSwitchBackgroundColor && userefx;
  ...
  useEffect(() => {
    console.log("useEffect[toggleRight, startOnLeft]");
